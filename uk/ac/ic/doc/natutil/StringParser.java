...
public class StringParser {
  ...
  TypeSpecificParser getParser(Class paramClass) {
    synchronized (this._parsers) {
      return (TypeSpecificParser)this._parsers.get(paramClass);
    } 
  }
  
  public void addParser(Class paramClass, TypeSpecificParser paramTypeSpecificParser) {
    synchronized (this._parsers) {
      this._parsers.put(paramClass, paramTypeSpecificParser);
    } 
  }
  
  public Object newObject(Class paramClass, String paramString) throws IllegalArgumentException {
    TypeSpecificParser typeSpecificParser = getParser(paramClass);
    if (typeSpecificParser != null)
      return typeSpecificParser.parse(paramString); 
    ...
    throw new IllegalArgumentException(
        "cannot convert \"" + paramString + "\" to instance of class " + 
        paramClass.getName());
  }
  
  public Object newObject(Class paramClass, List paramList) throws IllegalArgumentException {
    Constructor[] arrayOfConstructor = (Constructor[])paramClass.getConstructors();
    for (byte b = 0; b < arrayOfConstructor.length; b++) {
      Class[] arrayOfClass = arrayOfConstructor[b].getParameterTypes();
    ...
    throw new IllegalArgumentException(
        "failed to find a suitable constructor of class " + paramClass.getName());
  }
  
  public static interface TypeSpecificParser {
