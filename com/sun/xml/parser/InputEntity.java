...
final class InputEntity implements Locator {
  ...
  public void init(InputSource paramInputSource, String paramString, InputEntity paramInputEntity, boolean paramBoolean) throws IOException, SAXException {
    ...
    if (this.reader == null) {
      ...
      if (inputStream == null) {
        this.reader = XmlReader.createReader((
            new URL(paramInputSource.getSystemId()))
            .openStream());
      } else if (paramInputSource.getEncoding() != null) {
        this.reader = XmlReader.createReader(
            paramInputSource.getByteStream(), 
            paramInputSource.getEncoding());
      } else {
  ...
  private boolean checkSurrogatePair(int paramInt) throws SAXException {
    ...
    if (c1 >= '?' && c1 < '?' && c2 >= '?' && c2 <= '?')
      return true; 
    fatal("P-074", new Object[] { Integer.toHexString(c1 & Character.MAX_VALUE), 
          Integer.toHexString(c2 & Character.MAX_VALUE) });
    return false;
  }
    ...
    while (true) {
      ...
      switch (c) {
        ...
        case '\r':
          ...
          if (!isInternal())
            this.lineNumber++; 
          paramDocumentHandler.ignorableWhitespace(this.buf, i, 
              this.start - 1 - i);
  ...
  public String rememberText() {
    ...
    if (this.rememberedText != null) {
      this.rememberedText.append(this.buf, this.startRemember, 
          this.start - this.startRemember);
      str = this.rememberedText.toString();
    } else {
      str = new String(this.buf, this.startRemember, 
          this.start - this.startRemember);
    } 
  ...
  private void fatal(String paramString, Object[] paramArrayOfObject) throws SAXException {
    SAXParseException sAXParseException = new SAXParseException(
        Parser.messages.getMessage(this.locale, paramString, paramArrayOfObject), 
        this);
    close();
