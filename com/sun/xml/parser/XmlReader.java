...
final class XmlReader extends Reader {
  ...
  public static Reader createReader(InputStream paramInputStream, String paramString) throws IOException {
    ...
    if ("UTF-8".equalsIgnoreCase(paramString) || 
      "UTF8".equalsIgnoreCase(paramString))
      return new Utf8Reader(paramInputStream); 
    if ("US-ASCII".equalsIgnoreCase(paramString) || 
    ...
    if (i == 4)
      switch (arrayOfByte[0] & 0xFF) {
        ...
        case 60:
          ...
          break;
        case 76:
          if (arrayOfByte[1] == 111 && (
            0xFF & arrayOfByte[2]) == 167 && (
            0xFF & arrayOfByte[3]) == 148) {
  ...
  private void useEncodingDecl(PushbackInputStream paramPushbackInputStream, String paramString) throws IOException {
    ...
    InputStreamReader inputStreamReader = new InputStreamReader(
        new ByteArrayInputStream(arrayOfByte, 4, i), 
        paramString);
    int j;
        ...
        try {
          if ((this.buffer[j] & 0xE0) == 192) {
            ...
            throw new CharConversionException(
                ...
                "Unconvertible UTF-8 character beginning with 0x" + 
                Integer.toHexString(
                  this.buffer[this.start] & 0xFF));
          } 
        } catch (ArrayIndexOutOfBoundsException arrayIndexOutOfBoundsException) {
          i = 0;
        } 
        if (j > this.finish) {
          System.arraycopy(this.buffer, this.start, 
              this.buffer, 0, this.finish - this.start);
          this.finish -= this.start;
          this.start = 0;
          j = this.instream.read(this.buffer, this.finish, 
              this.buffer.length - this.finish);
          if (j < 0) {
            close();
            throw new CharConversionException(
                "Partial UTF-8 char");
