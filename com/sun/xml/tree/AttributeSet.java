...
final class AttributeSet implements NamedNodeMap, XmlWritable {
  ...
  AttributeSet(AttributeList paramAttributeList) throws DOMException {
    ...
    for (byte b = 0; b < i; b++)
      this.list.addElement(new AttributeNode(
            paramAttributeList.getName(b), 
            paramAttributeList.getValue(b), 
            (attributeListEx == null) ? true : 
            
            attributeListEx.isSpecified(b), 
            (attributeListEx == null) ? 
            null : 
            attributeListEx.getDefault(b))); 
    ...
    for (byte b = 0; b < i; b++) {
      ...
      if (node.getNodeName().equals(paramString)) {
        ...
        if (attributeNode.getDefaultValue() != null) {
          ...
          attributeNode.setOwnerDocument(
              (XmlDocument)this.nameScope.getOwnerDocument());
  ...
  public Node setNamedItem(Node paramNode) throws DOMException {
    ...
    if (!(paramNode instanceof AttributeNode) || 
      paramNode.getOwnerDocument() != 
      this.nameScope.getOwnerDocument())
