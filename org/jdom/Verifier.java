...
public final class Verifier {
  ...
  public static void main(String[] args) {
    for (int i = 0; i < 65536; i++) {
      if (isXMLLetter((char)i) != 
        isXMLLetterOld((char)i))
        System.out.println("isXMLLetter mismatch: " + i + 
            " hex: " + Integer.toHexString(i)); 
    } 
    for (int j = 0; j < 65536; j++) {
      if (isXMLDigit((char)j) != 
        isXMLDigitOld((char)j))
        System.out.println("isXMLDigit mismatch: " + j + 
            " hex: " + Integer.toHexString(j)); 
    } 
    for (int k = 0; k < 65536; k++) {
      if (isXMLCombiningChar((char)k) != 
        isXMLCombiningCharOld((char)k))
        System.out.println("isXMLCombiningChar mismatch: " + k + 
            " hex: " + Integer.toHexString(k)); 
    } 
    for (int m = 0; m < 65536; m++) {
      if (isXMLExtender((char)m) != 
        isXMLExtenderOld((char)m))
        System.out.println("isXMLExtender mismatch: " + m + 
            " hex: " + Integer.toHexString(m)); 
    } 
    for (int n = 0; n < 65536; n++) {
      if (isXMLCharacter((char)n) != 
        isXMLCharacterOld((char)n))
        System.out.println("isXMLCharacter mismatch: " + n + 
