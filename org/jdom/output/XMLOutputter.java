...
public class XMLOutputter implements Cloneable {
  ...
  protected void printCDATA(CDATA cdata, Writer out) throws IOException {
    String str = this.currentFormat.textNormalize ? 
      cdata.getTextNormalize() : (
      this.currentFormat.textTrim ? cdata.getText().trim() : 
      cdata.getText());
    out.write("<![CDATA[");
    out.write(str);
    out.write("]]>");
  ...
  protected void printText(Text text, Writer out) throws IOException {
    String str = this.currentFormat.textNormalize ? 
      ...
      this.currentFormat.textTrim ? text.getText().trim() : 
      text.getText());
    ...
    if (start < content.size()) {
      ...
      for (int i = start; i < end; i++) {
        ...
        if (next != null && !"".equals(next)) {
          if (previous != null && (
            this.currentFormat.textNormalize || 
            this.currentFormat.textTrim) && (
            endsWithWhite(previous) || 
  ...
  private int skipLeadingWhite(List content, int start) {
    if (start < 0)
      start = 0; 
    int index = start;
    if (this.currentFormat.trimAllWhite || 
      this.currentFormat.textNormalize || 
      this.currentFormat.textTrim || 
      this.currentFormat.newlines)
      while (index < content.size() && 
        isAllWhitespace(content.get(index)))
        index++;  
    return index;
  }
  ...
  private int skipTrialingWhite(List content, int start) {
    ...
    if (this.currentFormat.trimAllWhite || 
      ...
      this.currentFormat.newlines)
      while (index >= 0 && 
        isAllWhitespace(content.get(index - 1)))
        index--;  
    return index;
  }
  ...
  private int nextNonText(List content, int start) {
    ...
    int index = start;
    while (index < content.size() && (
      content.get(index) instanceof CDATA || 
  ...
  public String toString() {
    ...
    return 
      "XMLOutputter[omitDeclaration = " + this.omitDeclaration + ", " + 
      "encoding = " + this.encoding + ", " + 
      "omitEncoding = " + this.omitEncoding + ", " + 
      "indent = '" + this.defaultFormat.indent + "'" + ", " + 
      "expandEmptyElements = " + this.defaultFormat.expandEmptyElements + ", " + 
      "newlines = " + this.defaultFormat.newlines + ", " + 
      "lineSeparator = '" + buffer.toString() + "', " + 
      "trimAllWhite = " + this.defaultFormat.trimAllWhite + 
      "textTrim = " + this.defaultFormat.textTrim + 
      "textNormalize = " + this.defaultFormat.textNormalize + "]";
  }
